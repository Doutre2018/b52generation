...
public:
	...
	Polygon2d translated(double x, double y);
	Polygon2d rotated(double orientationDegrees);
	Polygon2d scaled(double relativeScale);
	Polygon2d transformed(double tX, double tY, double orientationDegrees, double relativeScale);
	Polygon2d mapped(Transform2d const & t);
	...
	// Drawing tools
	void drawBoundingBox(ConsoleImage & image, ConsoleColor const & boundingBoxColor);
	void drawVertices(ConsoleImage & image, size_t vertexSize, ConsoleColor const & vertexColor);
	void drawVertices(ConsoleImage & image, size_t vertexSize, ConsoleColor const & vertexColor, size_t nonConvexVertexSize, ConsoleColor const & nonConvexVertexColor);
	void drawSegments(ConsoleImage & image, ConsoleColor const & segmentsColor);
	void drawSegments(ConsoleImage & image, ConsoleColor const & segmentsColor, ConsoleColor const & intersectingSegmentsColor);
	void draw(ConsoleImage & image, ConsoleColor const & segmentsColor, ConsoleColor const & boundingBoxColor = ConsoleColor::invalid());
	void draw(ConsoleImage & image, size_t vertexSize, ConsoleColor const & vertexColor, ConsoleColor const & segmentsColor, ConsoleColor const & boundingBoxColor = ConsoleColor::invalid());
	void draw(ConsoleImage & image, size_t vertexSize, ConsoleColor const & vertexColor, size_t nonConvexVertexSize, ConsoleColor const & nonConvexVertexColor, ConsoleColor const & segmentsColor, ConsoleColor const & boundingBoxColor = ConsoleColor::invalid());
	void drawIntersect(ConsoleImage & image, ConsoleColor const & segmentsColor, ConsoleColor const & intersectingSegmentColor, ConsoleColor const & boundingBoxColor = ConsoleColor::invalid());
	void drawIntersect(ConsoleImage & image, size_t vertexSize, ConsoleColor const & vertexColor, ConsoleColor const & segmentsColor, ConsoleColor const & intersectingSegmentsColor, ConsoleColor const & boundingBoxColor = ConsoleColor::invalid());
	void drawIntersect(ConsoleImage & image, size_t vertexSize, ConsoleColor const & vertexColor, size_t nonConvexVertexSize, ConsoleColor const & nonConvexVertexColor, ConsoleColor const & segmentsColor, ConsoleColor const & intersectingSegmentsColor, ConsoleColor const & boundingBoxColor = ConsoleColor::invalid());

	// Point interaction tools
	// Return true if p is inside the polygon (borderIncluded is not used)
	...
	//  - the distance is degative if the point is inside the polygon
